...
public class Game {
    ...
    public void fazTudo(String comando, String id, Lista Lista,String path) throws Exception{
    ...
    public void fazTudoException(String comando, String pos, String id, Lista Lista,String path) throws Exception{
        int idInt = Integer.parseInt(id);
    ...
    public static void main(String[] args)  throws Exception {
        ...
        while(!Game.isFim(id)){ //lendo e criando a lista
            ...
            lista.inserirFim(Game.getAppId());
            id = MyIO.readLine();
        }
        numeroLinhas = MyIO.readInt();
        for(int i = 0; i<numeroLinhas; i++){
            idString = MyIO.readLine();
            if(idString.length()>2 && idString.charAt(1)!='*'){
                stringSeparada = Game.separaIdString(idString); //non static = colocar Game.funcao
                Game.fazTudo(stringSeparada[0], stringSeparada[1], lista,path);
            }
            else if(idString.charAt(1)=='*' && idString.length()>=5){
                stringSeparada = Game.separaIdString(idString); //non static = colocar Game.funcao
                try{
                    Game.fazTudoException(stringSeparada[0], stringSeparada[1], stringSeparada[2], lista,path);} //funcao switch case
                    catch(java.lang.ArrayIndexOutOfBoundsException e){
                        Game.fazTudoException(stringSeparada[0], stringSeparada[1], "0", lista,path);
                    }
            }
            
            else{
                stringSeparada[0]=idString;
                Game.fazTudo(stringSeparada[0], "0", lista,path);
            }
            
        }

        listaCheia = lista.mostrar();
        String fraseInteira;
